-- Применение миграции user_query_logs через Supabase Dashboard
...
DROP POLICY IF EXISTS "Users can view their own logs" ON user_query_logs;
DROP POLICY IF EXISTS "System can insert logs" ON user_query_logs;

CREATE POLICY "Users can view their own logs"
  ON user_query_logs FOR SELECT
  USING (auth.uid() = user_id);

-- Политика для вставки логов через service_role key (bypass RLS)
-- Service role key автоматически bypass RLS, но политика нужна для явной настройки
DROP POLICY IF EXISTS "System can insert logs" ON user_query_logs;
CREATE POLICY "System can insert logs"
  ON user_query_logs FOR INSERT
  WITH CHECK (true); -- Разрешаем вставку любых данных через service_role key
...
SELECT 
  '✅ Таблица user_query_logs создана успешно!' as status,
