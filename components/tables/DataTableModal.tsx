...
export default function DataTableModal({ id, sql, columns, rows, onClose, onMinimize, isMinimized, currentName }: DataTableModalProps) {
  ...
  // Отслеживаем открытие модального окна (только один раз)
  useEffect(() => {
    if (!isMinimized && !openedTracked) {
      const trackOpen = async () => {
        try {
          const jwt = localStorage.getItem('jwt');
          if (!jwt) return;

          const response = await fetch('/api/increment-table-opens', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${jwt}`,
            },
            body: JSON.stringify({ is_download: false }),
          });
  ...
  }, [isMinimized, openedTracked, id, onClose]);
  ...
  const downloadFile = async (content: string | Blob, filename: string, mimeType?: string) => {
    // Проверяем лимит и увеличиваем счетчик перед скачиванием
    try {
      const jwt = localStorage.getItem('jwt');
      if (jwt) {
        const response = await fetch('/api/increment-table-opens', {
          ...
          body: JSON.stringify({ is_download: true }),
        });
    ...
    // Если лимит не превышен, скачиваем файл
  ...
  const exportToSQL = async () => {
    ...
    await downloadFile(sqlContent, `table_export_${Date.now()}.sql`, "text/sql");
  };
  ...
  const exportToCSV = async () => {
    ...
    await downloadFile(csv, `table_export_${Date.now()}.csv`, "text/csv");
  };
  ...
  const exportToXLSX = async () => {
    ...
    // Сохраняем в буфер и скачиваем через downloadFile для проверки лимитов
    const xlsxBuffer = XLSX.write(workbook, { type: 'array', bookType: 'xlsx' });
    await downloadFile(new Blob([xlsxBuffer]), `table_export_${Date.now()}.xlsx`, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
  };
  ...
  const exportToXLS = async () => {
    ...
    // Сохраняем в буфер и скачиваем через downloadFile
    const xlsBuffer = XLSX.write(workbook, { type: 'array', bookType: 'xls' });
    await downloadFile(new Blob([xlsBuffer]), `table_export_${Date.now()}.xls`, "application/vnd.ms-excel");
  };
  ...
  const exportToJSON = async () => {
    ...
    await downloadFile(jsonContent, `table_export_${Date.now()}.json`, "application/json");
  };
  ...
  const exportToPDF = async () => {
    ...
    // Проверяем лимит перед открытием окна печати
    try {
      const jwt = localStorage.getItem('jwt');
      if (jwt) {
        const response = await fetch('/api/increment-table-opens', {
...
<body>
  ...
  <table>
    ...
    await downloadFile(htmlContent, `table_export_${Date.now()}.doc`, "application/msword");
  };

  // Экспорт в DOCX (используем тот же HTML формат)
  const exportToDOCX = async () => {
    await exportToDOC(); // DOCX можно открыть как DOC в большинстве случаев
  };
